    ...
    private void populatePositionCoordinates()
    {
        ...
        for (int i = 0; i < SnakeAndLadders.height; i++) {
            for (int j = 0; j < SnakeAndLadders.width; j++) {
                //x coordinates
                int xCord=0;
                if(i%2==0) //for even value: x increases
                {
                    xCord=j*SnakeAndLadders.tileSize + SnakeAndLadders.tileSize/2;

                }
                else //for odd value: x decreases
                {
                    xCord=SnakeAndLadders.tileSize*SnakeAndLadders.height - SnakeAndLadders.tileSize/2 - (j*SnakeAndLadders.tileSize);
                }
                //y coordinates
                int yCord=SnakeAndLadders.tileSize*SnakeAndLadders.height - SnakeAndLadders.tileSize/2 - (i*SnakeAndLadders.tileSize);
